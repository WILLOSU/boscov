...
export function ManageFilmes() {
    ...
    async function fetchFilmeData() {
      if (action === "edit" && id) {
        try {
          ...
          setErrorMessage("Não foi possível carregar os dados do filme. Tente novamente mais tarde.")
        } finally {
          setLoading(false)
        }
      } else if (action === "add") {
  ...
  function convertToFilmesData(data: FilmeFormValues): FilmesData {
    ...
    return {
      ...
      usuarioCriador: user?.id || (data.usuarioCriador ? Number(data.usuarioCriador) : undefined),
  ...
  const registerFilmesSubmit = async (data: FilmeFormValues) => {
    try {
      ...
      setErrorMessage("Erro ao cadastrar filme. Verifique os dados e tente novamente.")
  ...
  const editFilmesSubmit = async (data: FilmeFormValues) => {
    ...
    try {
      ...
      setErrorMessage("Erro ao atualizar filme. Verifique os dados e tente novamente.")
    ...
    <AddFilmesContainer>
      ...
      {loading ? (
        ...
        <form onSubmit={handleRegisterFilmes(action === "add" ? registerFilmesSubmit : editFilmesSubmit)}>
          ...
          <Input type="text" placeholder="URL do Poster do Filme" name="poster" register={registerFilmes} />
          {errosRegisterFilmes.poster && <ErrorSpan>{errosRegisterFilmes.poster.message}</ErrorSpan>}
          ...
          <Input type="number" placeholder="Status (1 = Ativo, 0 = Inativo)" name="status" register={registerFilmes} />
          ...
          <Button type="submit" disabled={loading}>
            {loading ? "Processando..." : action === "add" ? "Adicionar" : "Atualizar"}
          </Button>
        </form>
      )}
    </AddFilmesContainer>
