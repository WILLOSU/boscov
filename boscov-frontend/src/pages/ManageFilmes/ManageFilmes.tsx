...
export function ManageFilmes() {
    ...
    async function fetchFilmeData() {
      if (action === "edit" && id) {
        try {
          ...
          setValue("usuarioCriador", filme.usuarioCriador ?? 1);
        } catch (error) {
  ...
  function convertToFilmesData(data: FilmeFormValues): FilmesData {
    return {
      ...
      status: data.status === "1", // Converte para boolean (1 = true, outros valores = false)
      // Remova usuarioCriador se ele for definido pelo backend
    };
  }
    ...
    <AddFilmesContainer>
      ...
      {loading ? (
        ...
        >
          ...
          <Input
            type="number"
            placeholder="Status (1 = Ativo, 2 = Inativo)"
            name="status"
            register={registerFilmes}
          />
